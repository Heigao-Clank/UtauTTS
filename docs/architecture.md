...
PitchFactorを与えない`waveform`を発音明瞭度の固定基準とします。韻律レンダラの評価では、この基準より文章の聞き取りやすさを落とさないことを最初の採用条件にします。

### waveform-vowel-pitch（実験的）

通常の`waveform`と同じ時間配置を先に作り、`oto.ini`固定範囲より後ろの母音だけへPitchFactorを適用します。固定範囲以前のサンプルは変更せず、母音側12msだけを前向きにクロスフェードします。

現在はリサンプルとWSOLAを組み合わせた最小検証版です。明瞭度を維持できることを確認してから、ピッチマークに基づくPSOLAへ置き換えます。標準レンダラではありません。
