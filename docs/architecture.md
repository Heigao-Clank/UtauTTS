...
`connection-train`は音源・groupを跨がない分割を行い、話者非依存の境界差分から小規模なロジスティック回帰を学習します。`--join-model`を指定すると、予測確率のlogitを−2から＋2へ制限して−8から＋8の音響join scoreへ変換し、同一録音の順方向継続ボーナスと合わせてViterbi探索へ使用します。モデルを省略した場合は手設計join scoreへ戻ります。
...
手設計join scoreは、同じ録音内の自然な順序を優遇し、接続前後30msの正規化スペクトル、RMS、F0、有声・無声の一致を比較する決定的なベースラインです。学習モデルv2はこれらの差分にピッチ同期波形相関を加えます。WAVを解析できない場合は局所スコアだけで探索を継続します。探索量を制限するため各位置はtarget score上位32候補へ事前選別します。
