...
`connection-dataset`はUTAUボイスバンクから、join cost学習用の弱教師付き境界ペアをJSONLで生成します。同じWAV内でoffset順に隣り合う原音を正例、右側を同じ発音末尾の別録音原音へ差し替えたものを負例とします。
...
モデルv2は差分スペクトル10帯域、スペクトル平均差、RMS差、F0差、voicing不一致、波形相関を入力する正規化ロジスティック回帰です。波形は240点へ正規化し、±5msの位相ずれを探索した最大相関を使います。クラス数の偏りは学習重みで補正します。モデルJSONには正規化係数、重み、学習音源、validationのaccuracy、balanced accuracy、AUC、log lossを保存します。`<out>.split.json`で分割後・欠損除外後の件数と音源名を監査できます。モデルv1も推論互換のため読み込めます。
