...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            for(const cents of [-300,0,300]){ctx.strokeStyle=cents===0?root.axisColor:root.gridColor;ctx.setLineDash(cents===0?[]:[4,5]);ctx.beginPath();ctx.moveTo(0,center-cents*scale);ctx.lineTo(width,center-cents*scale);ctx.stroke()}
            ctx.setLineDash([]);if(!root.points.length)return;ctx.strokeStyle=root.accentColor;ctx.fillStyle=root.accentColor;ctx.lineWidth=2;ctx.beginPath()
    ...
    Row {
        ...
        Repeater { model: root.morae; delegate: Text { required property var modelData;width:root.morae.length?canvas.width/root.morae.length:0;text:modelData.mora||"・";horizontalAlignment:Text.AlignHCenter;color:root.labelColor;elide:Text.ElideRight } }
    }
    MouseArea {
        anchors.fill: canvas
