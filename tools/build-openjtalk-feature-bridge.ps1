...
if (-not (Test-Path -LiteralPath $pythonPackage)) {
    & python -m pip install --target $packageRoot 'pyopenjtalk==0.4.1'
...
if (-not (Test-Path -LiteralPath $dictionaryPath)) {
    ...
    try {
        ...
        & python -c "import pyopenjtalk; pyopenjtalk.run_frontend('テスト')"
...
if (-not (Test-Path -LiteralPath (Join-Path $pyInstallerRoot 'PyInstaller'))) {
    & python -m pip install --target $pyInstallerRoot 'pyinstaller==6.16.0'
...
& python (Join-Path $root 'tools/verify-openjtalk-feature-bridge.py') `
