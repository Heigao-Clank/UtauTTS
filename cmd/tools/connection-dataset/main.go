...
func main() {
	...
	flag.StringVar(&config.NegativeStrategy, "negative-strategy", "hard", "negative selection: hard or rotating")
	...
	if config.NegativeStrategy != "hard" && config.NegativeStrategy != "rotating" {
		log.Fatal("--negative-strategy must be hard or rotating")
	}
